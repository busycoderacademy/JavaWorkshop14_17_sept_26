...
import com.bankapp.exceptions.JpaException;
...
public class AccountRepoJpaImpl implements  AccountRepo {
    ...
    public List<Account> getAllAccounts() {
        System.out.println("getAllAccounts() called using JPA");
        try{
            if(1==1)
                throw new JpaException("some bad jpql quaries");
        }catch (JpaException e){
            //i can wrap this SQL Ex and rethrow it form catch block
            throw new DataAcessException("some bad jpql quaries", e);
        }
